...
const serveBuffer = (req, res, buf, opt = {}) => {
	const {
		contentType,
		timeModified,
		etag,
	...
	if ('string' !== typeof contentType) {
		throw new Error('opt.contentType must a string')
	}
	if (!(timeModified instanceof Date)) {
		throw new Error('opt.timeModified must be a Date')
	}
	if (etag !== null && 'string' !== typeof etag) {
		throw new Error('opt.etag must a string or null')
	}
